...
//  EssentialFeedTests
//
//  Created by Muqeem.Ahmad on 05/06/23.
//
...
final class CacheFeedUseCaseTests: XCTestCase {
    ...
    func test_save_requestsNewCacheInsertionWithTimesStampOnSuccessfulDeletion() {
        let timestamp = Date()
        let items = [uniqueItem(), uniqueItem()]
    ...
    func test_failsOnInserrtionnError() {
        let items = [uniqueItem(), uniqueItem()]
        let (sut, store) = makeSUT()
    ...
    func test_successOnSuccessfulCacheInserrtionn() {
        let items = [uniqueItem(), uniqueItem()]
        let (sut, store) = makeSUT()
        let insertionError = anyNSError()
        let exp = expectation(description: "Wait for save completion")
        ...
        store.completeDeletionSuccessfully()
        store.completeInsertionCompleteSuccessfully()
        wait(for: [exp], timeout: 1.0)
